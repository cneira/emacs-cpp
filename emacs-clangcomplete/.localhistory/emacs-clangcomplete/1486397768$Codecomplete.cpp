...
const std::string  intellisense(CXCodeCompleteResults *compResults) {
	...
	unsigned numDiags = clang_codeCompleteGetNumDiagnostics(compResults);
	std::cout << "debug: complete: " << numDiags << " diagnostic(s)\n";
...
extern "C" {
	__declspec(dllexport) const char *clang_complete(char *filename,
		int line_number, int column_number,char* dir) {
		...
		CXTranslationUnit tu = clang_parseTranslationUnit(
			...
			const_cast<CXUnsavedFile *>(cxUnsavedFiles.data()),
			cxUnsavedFiles.size(),
			(size_t)parseTUOptions_);
		if (tu == NULL) {
		...
		CXCompilationDatabase db = clang_CompilationDatabase_fromDirectory(dir, &error);
		switch (error) {
		...
		compResults =
			clang_codeCompleteAt(tu, filename, line_number, column_number,
				const_cast<CXUnsavedFile *>(cxUnsavedFiles.data()), cxUnsavedFiles.size(), (clang_defaultCodeCompleteOptions() &
		...
		clang_sortCodeCompletionResults(compResults->Results,
